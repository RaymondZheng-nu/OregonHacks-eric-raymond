...
const variantStyles = {
  standard: {
    copy: "pt-20 pb-10 sm:pt-24 sm:pb-12",
  ...
  compact: {
    copy: "pt-14 pb-8 sm:pt-20 sm:pb-10",
    tagline: "text-sm",
...
export function Hero05({
  ...
  const taglineElement = tagline && (
    <p
      className={cn(
        "max-w-xs leading-relaxed tracking-tight text-muted-foreground",
        vs.tagline
      )}
    >
  ...
  const titleElement = title && (
    <h1
      className={cn(
        "text-balance font-sans font-semibold tracking-tight text-foreground",
        vs.title
      )}
    >
  ...
  const mediaElement = landscapeImage && (
    <div className="relative w-full overflow-hidden">
      ...
      >
        <div
          aria-hidden
          className="pointer-events-none absolute inset-0 z-10 bg-background/15 mix-blend-overlay dark:bg-background/30"
        />
  ...
  return (
    <section className="relative isolate w-full overflow-hidden bg-background">
      <motion.div
        className={cn(
          "relative z-10 mx-auto grid max-w-[1400px] grid-cols-1 px-4 lg:grid-cols-12",
          vs.copy,
          vs.grid
        )}
